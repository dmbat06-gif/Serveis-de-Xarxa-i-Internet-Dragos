...
* **¿En qué menú de tu dispositivo móvil (Android/iOS) podrías forzar el uso de unos DNS específicos para tu conexión Wi-Fi?**
  * Ajustes - WiFi - Seleccionar red - Editar - Opciones avanzadas - Ajustes de IP - Estática
...
* **Muestra una captura de pantalla de algunas direcciones almacenadas en la caché de tu equipo (`ipconfig /displaydns` en Windows o `resolvectl statistics` en Linux):**
 ...
 ![Imagen_displaydns](./img/displaydns.png)
...
![Imagen_dig_aliexpress](./img/dig_aliexpress.png)
...
![Imagen_dig_short_aliexpress](./img/dig_short_aliexpress.png)
  * *¿Por qué es útil este formato en scripts de Bash?*
    * Es útil porque proporciona una salida simplificada. Esto permite automatizar tareas como comprobar direcciones IP, validar resoluciones DNS o realizar comprobaciones de disponibilidad.

* **MX (`dig MX aliexpress.com`):**

  
![Imagen_dig_mx_aliexpress](./img/dig_mx_aliexpress.png)
...
![Imagen_dig_ns_aliexpress](./img/dig_ns_aliexpress.png)
...
* **Realiza una consulta a un dominio cualquiera. Observa el valor TTL (Time To Live). Vuelve a realizar la consulta a los 5 segundos:**
  * *Resultado del TTL inicial:* 271 segundos
  * *Resultado del TTL tras 5 segundos:* 265 segundos
  * *¿Qué ha pasado con el valor numérico del TTL? ¿Qué nos demuestra esto sobre el origen de la respuesta (Caché vs Servidor Autoritativo)?*
    * El valor del TTL ha disminuido casi 5 segundos. Esto indica que la respuesta estaba siendo servida desde una caché DNS de un servidor recursivo, ya que el TTL representa el tiempo restante durante el cual el registro puede permanecer almacenado en caché.
    * Si la consulta se realizara directamente contra un servidor autoritativo, normalmente veríamos el TTL establecido originalmente para ese registro, mientras que un resolvedor que tiene la respuesta almacenada en caché devuelve el tiempo restante. Por tanto, la disminución del TTL entre consultas es un indicio de que la respuesta procede de una caché y no de una consulta nueva al servidor autoritativo.


### 3.3. Trazabilidad Completa (`+trace`)
* **Ejecuta el comando `dig +trace aliexpress.com`:**

![Imagen_dig_+trace_aliexpress](./img/dig_+trace_aliexpress.png)
...
* **Explica paso a paso cómo tu ordenador contacta primero con los Root Servers (`.`), luego con los servidores del TLD (`.com`) y finalmente con los servidores autoritativos del dominio:**
  1. **Root Servers (`.`):** Indican qué servidores gestionan el TLD .com
  2. **TLD Servers (`.com`):** Indican cuáles son los servidores autoritativos de aliexpress.com
  3. **Servidores Autoritativos:** Proporcionan la información DNS oficial del dominio, como su dirección IP.
